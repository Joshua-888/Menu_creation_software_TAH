...
describe("policy application + probability in planning", () => {
  ...
  it("strips dips from sandwich after fan-out; keeps dips on pommes", () => {
    ...
    expect(fan.fanOutMenus).toContain("50");
    expect(fan.fanOutMenus).toContain("48");
    ...
    const sandwich = filtered.traces.find((t) => t.menuNumber === "50")!;
    const pommes = filtered.traces.find((t) => t.menuNumber === "48")!;
