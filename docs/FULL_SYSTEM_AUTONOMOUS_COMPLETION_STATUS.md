...
- Non-blocking finding surfaced by Builder: `.a0proj/` is not in `.gitignore` (accidentally staged once, correctly reset out of the commit) — flagged for a future housekeeping WP, not fixed here (out of scope).
