...
3. Live writes are on with `TAH_ADMIN_*` + host allowlist (`PORTAL_LIVE_WRITES=0` to stop). Products are storefront-visible by default (`Aktiv?` checked); set `PORTAL_CREATE_HIDDEN=1` to keep Skjult.
