...
const lines = p6.lines
  .filter((l) => l.y >= 400 && l.y <= 580)
  .map((l) => `${l.y}|${JSON.stringify(l.text)}`);
const items = p6.items
  .filter((it) => it.y >= 400 && it.y <= 580)
  .sort((a, b) => b.y - a.y || a.x - b.x)
  .map((it) => `${it.y}|${it.x}|${JSON.stringify(it.str)}`);
writeFileSync(
  "runs/m5h2-p6-indisk.txt",
  ["LINES", ...lines, "ITEMS", ...items].join("\n"),
  "utf8",
);
